...
class Action {
    ...
    /**
     * Load saved data into this action
     * @param {Object} actionData - The saved action data
     */
    loadData(actionData) {
        // Set basic form values
        document.getElementById(`actionName-${this.locationId}-${this.id}`).value = actionData.name || '';
        
        if (actionData.description) {
            document.getElementById(`description-${this.locationId}-${this.id}`).value = actionData.description;
        }
        
        document.getElementById(`quantity-${this.locationId}-${this.id}`).value = actionData.quantity || 1;
        document.getElementById(`unit-${this.locationId}-${this.id}`).value = actionData.unit || 'Count';
        document.getElementById(`unitPrice-${this.locationId}-${this.id}`).value = actionData.unitPrice || 0;
        
        // Restore selected action ID and materials data
        if (actionData.actionId) {
            this.selectedActionId = actionData.actionId;
        }
        
        if (actionData.materials) {
            this.materials = actionData.materials;
        }
        
        if (actionData.subtasks) {
            this.subtasks = actionData.subtasks;
        }
        
        if (actionData.materialCost !== undefined) {
            this.totalMaterialCost = actionData.materialCost;
        }
        
        // If we have a selected action ID, recalculate materials and subtasks
        // This ensures current prices and calculations
        if (this.selectedActionId) {
            this.calculateMaterialsAndSubtasks();
        }
